...
# TODO: Telco IDs, Telco Kategorien

def create_onehot_schema(df_full, cat_cols=None):
    import numpy as np

    # remove IDs
    id_like_cols = [c for c in df_full.columns if "id" in c.lower()]
    df_full = df_full.drop(columns=id_like_cols, errors="ignore")

    if cat_cols is None:
        cat_cols = df_full.select_dtypes(exclude=[np.number]).columns.tolist()

    df_full_oh = pd.get_dummies(df_full, columns=cat_cols, drop_first=False)

    df_full_oh = df_full_oh.astype(np.float32)

    schema_cols = df_full_oh.columns

    return df_full_oh, schema_cols, cat_cols


def apply_onehot_schema(df, schema_cols, cat_cols):
    """
    Apply One-Hot schema.
    """
    df_oh = pd.get_dummies(df, columns=cat_cols, drop_first=False)

    # align to schema_cols
    df_oh = df_oh.reindex(columns=schema_cols, fill_value=0)

    df_oh = df_oh.astype(np.float32)
