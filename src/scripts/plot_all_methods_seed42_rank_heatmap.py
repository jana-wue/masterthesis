from __future__ import annotations

import os
from pathlib import Path
import sys

PROJECT_ROOT = Path(__file__).resolve().parents[2]
OUTPUT_DIR = PROJECT_ROOT / "data" / "results" / "figures"
MPL_CONFIG_DIR = PROJECT_ROOT / "data" / "results" / ".mpl-cache"

os.environ.setdefault("MPLCONFIGDIR", str(MPL_CONFIG_DIR))

import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from matplotlib.axes import Axes
import pandas as pd
import seaborn as sns

plt.rcParams.update(
    {
        "font.family": "serif",
        "font.serif": ["Times New Roman", "Times", "DejaVu Serif"],
        "mathtext.fontset": "dejavuserif",
    }
)

if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from src.paths import DATA_RESULTS


INPUT_PATH = DATA_RESULTS / "benchmark_all_completed_results.csv"
OUTPUT_PNG = OUTPUT_DIR / "all_methods_seed42_rank_heatmap.png"
OUTPUT_PDF = OUTPUT_DIR / "all_methods_seed42_rank_heatmap.pdf"
OUTPUT_CSV = OUTPUT_DIR / "all_methods_seed42_rank_heatmap_table.csv"

DATASET_ORDER = ["German Credit Card", "German Statlog", "Telco"]
SCENARIO_ORDER = ["MAR_TARGET", "MCAR_TARGET", "MNAR_TARGET"]
SCENARIO_LABELS = {
    "MAR_TARGET": "MAR-T",
    "MCAR_TARGET": "MCAR-T",
    "MNAR_TARGET": "MNAR-T",
}

ROW_ORDER = [
    "meanmode",
    "medianmode",
    "mice",
    "missforest",
    "dae",
    "llm_prompt::llama",
    "llm_finetuned::llama",
    "llm_prompt::mistral",
    "llm_finetuned::mistral",
    "llm_prompt::qwen",
    "llm_finetuned::qwen",
]

ROW_LABELS = {
    "meanmode": "Mean/Mode",
    "medianmode": "Median/Mode",
    "mice": "MICE",
    "missforest": "MissForest",
    "dae": "DAE",
    "llm_prompt::llama": "Llama 3.1 Prompt",
    "llm_finetuned::llama": "Llama 3.1 Finetuned",
    "llm_prompt::mistral": "Mistral Prompt",
    "llm_finetuned::mistral": "Mistral Finetuned",
    "llm_prompt::qwen": "Qwen 2.5 Prompt",
    "llm_finetuned::qwen": "Qwen 2.5 Finetuned",
}


def _load_comparison_rows() -> pd.DataFrame:
    """Load comparison rows."""
    df = pd.read_csv(INPUT_PATH)
    work = df[
        (df["status"] == "success")
        & (df["seed"] == 42)
        & (df["rate_pct"] == 10)
        & (df["scenario_family"].isin(SCENARIO_ORDER))
        & (df["method_family"].isin(["classical", "llm_prompt", "llm_finetuned"]))
    ].copy()
    if work.empty:
        raise ValueError("No successful seed-42 rows found for the all-methods comparison.")
    return work


def _build_summary(comparison_df: pd.DataFrame) -> pd.DataFrame:
    """Build summary."""
    work = comparison_df.copy()

    classical_mask = work["method_family"] == "classical"
    work.loc[classical_mask, "row_key"] = work.loc[classical_mask, "method_key"].astype(str)

    llm_mask = work["method_family"].isin(["llm_prompt", "llm_finetuned"])
    work.loc[llm_mask, "row_key"] = (
        work.loc[llm_mask, "method_family"].astype(str)
        + "::"
        + work.loc[llm_mask, "model_key"].astype(str)
    )

    summary = (
        work.groupby(["dataset_name", "scenario_family", "row_key"], as_index=False)["mean_nrmse"]
        .mean()
        .rename(columns={"mean_nrmse": "mean_nrmse_avg"})
    )
    summary["rank"] = (
        summary.groupby(["dataset_name", "scenario_family"])["mean_nrmse_avg"]
        .rank(method="min", ascending=True)
        .astype(int)
    )
    return summary


def _task_sort_key(task: tuple[str, str]) -> tuple[int, int]:
    """Handle task sort key."""
    dataset_name, scenario_family = task
    return (
        DATASET_ORDER.index(dataset_name),
        SCENARIO_ORDER.index(scenario_family),
    )


def _build_rank_matrix(summary: pd.DataFrame) -> tuple[pd.DataFrame, list[tuple[str, str]]]:
    """Build rank matrix."""
    tasks = sorted(
        {(row["dataset_name"], row["scenario_family"]) for _, row in summary.iterrows()},
        key=_task_sort_key,
    )

    matrix = (
        summary.pivot_table(
            index="row_key",
            columns=["dataset_name", "scenario_family"],
            values="rank",
            aggfunc="first",
        )
        .reindex(index=ROW_ORDER)
        .reindex(columns=pd.MultiIndex.from_tuples(tasks))
    )

    missing_rows = [row for row in ROW_ORDER if row not in matrix.index]
    if missing_rows:
        raise ValueError(f"Missing expected comparison rows: {missing_rows}")

    matrix.index = [ROW_LABELS[key] for key in matrix.index]
    return matrix, tasks


def _build_display_labels(tasks: list[tuple[str, str]]) -> list[str]:
    """Build display labels."""
    return [f"{SCENARIO_LABELS[scenario_family]}\n10%" for _, scenario_family in tasks]


def _draw_dataset_group_labels(ax: Axes, tasks: list[tuple[str, str]]) -> None:
    """Draw dataset group labels."""
    dataset_positions: dict[str, list[int]] = {}
    for idx, (dataset_name, _) in enumerate(tasks):
        dataset_positions.setdefault(dataset_name, []).append(idx)

    for dataset_name in DATASET_ORDER:
        positions = dataset_positions.get(dataset_name, [])
        if not positions:
            continue
        center = (positions[0] + positions[-1] + 1) / 2.0
        ax.text(
            center,
            1.12,
            dataset_name,
            ha="center",
            va="center",
            fontsize=10,
            fontweight="semibold",
            color="#333333",
            clip_on=False,
            transform=ax.get_xaxis_transform(),
        )

    for split_after in [3, 6]:
        ax.axvline(split_after, color="#7a7a7a", linestyle=(0, (6, 6)), linewidth=1.2, alpha=0.9)

    for split_after in [5, 7, 9]:
        ax.axhline(split_after, color="#b5b5b5", linestyle=(0, (4, 4)), linewidth=0.9, alpha=0.8)


def _plot_heatmap(rank_matrix: pd.DataFrame, tasks: list[tuple[str, str]]) -> None:
    """Plot heatmap."""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    MPL_CONFIG_DIR.mkdir(parents=True, exist_ok=True)

    fig_width = max(10.5, len(tasks) * 0.9)
    fig_height = 7.1
    fig, ax = plt.subplots(figsize=(fig_width, fig_height))

    cmap = sns.color_palette("plasma_r", as_cmap=True)
    sns.heatmap(
        rank_matrix,
        ax=ax,
        cmap=cmap,
        vmin=1,
        vmax=len(rank_matrix.index),
        annot=True,
        fmt=".0f",
        cbar=False,
        linewidths=0.6,
        linecolor="white",
        annot_kws={"fontsize": 9.5, "fontweight": "semibold"},
    )

    fig.suptitle("All Methods on Shared Seed-42 Tasks: Task Rankings", fontsize=20, y=0.965)
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_xticklabels(_build_display_labels(tasks), rotation=55, ha="right", fontsize=10)
    ax.set_yticklabels(ax.get_yticklabels(), rotation=0, fontsize=11)
    _draw_dataset_group_labels(ax, tasks)

    plt.tight_layout(rect=(0, 0, 1, 0.945))
    fig.savefig(OUTPUT_PNG, dpi=300, bbox_inches="tight")
    fig.savefig(OUTPUT_PDF, bbox_inches="tight")
    plt.close(fig)


def main() -> None:
    """Run the script entry point."""
    comparison_df = _load_comparison_rows()
    summary = _build_summary(comparison_df)
    rank_matrix, tasks = _build_rank_matrix(summary)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    MPL_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    rank_matrix.to_csv(OUTPUT_CSV)
    _plot_heatmap(rank_matrix=rank_matrix, tasks=tasks)

    print("=" * 80)
    print("ALL METHODS SEED-42 RANK HEATMAP WRITTEN")
    print("=" * 80)
    print(f"Input CSV : {INPUT_PATH.resolve()}")
    print(f"Output PNG: {OUTPUT_PNG.resolve()}")
    print(f"Output PDF: {OUTPUT_PDF.resolve()}")
    print(f"Output CSV: {OUTPUT_CSV.resolve()}")


if __name__ == "__main__":
    main()
