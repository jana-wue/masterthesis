...
OUTPUT_PNG = OUTPUT_DIR / "classical_method_rank_heatmap.png"
OUTPUT_PDF = OUTPUT_DIR / "classical_method_rank_heatmap.pdf"
OUTPUT_CSV = OUTPUT_DIR / "classical_method_rank_heatmap_table.csv"
...
def _build_task_summary(classical_df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["rank"] = (
        summary.groupby(["dataset_name", "scenario_family", "rate_pct"])["mean_nrmse_avg"]
...
def _build_rank_matrix(summary: pd.DataFrame) -> tuple[pd.DataFrame, list[tuple[str, str, int]]]:
    """Build rank matrix."""
    tasks = sorted(
        {
...
def _draw_dataset_group_labels(ax: Axes, tasks: list[tuple[str, str, int]]) -> None:
    ...
    for split_after in [8, 16]:
        ax.axvline(split_after, color="#7f7f7f", linestyle=(0, (6, 6)), linewidth=1.2, alpha=0.9)


def _plot_heatmap(rank_matrix: pd.DataFrame, tasks: list[tuple[str, str, int]]) -> None:
    """Plot heatmap."""
    ...
    fig.suptitle("Classical Method Rankings on Each Benchmark Task", fontsize=20, y=0.965)
    ax.set_xlabel("")
    ax.set_ylabel("")
...
def main() -> None:
    ...
    export_table = rank_matrix.copy()
    export_table.to_csv(OUTPUT_CSV)
    _plot_heatmap(rank_matrix=rank_matrix, tasks=tasks)

    print("=" * 80)
    print("CLASSICAL METHOD RANK HEATMAP WRITTEN")
    print("=" * 80)
